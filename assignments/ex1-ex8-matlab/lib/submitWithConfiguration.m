...
function response = submitParts(conf, email, token, parts)
  ...
  % remove the incorrected string
%   responseBody = responseBody(find(responseBody=='{'):end);
...
function response = getResponse(url, body)
  ...
  if ispc
    ...
    json_command = sprintf('env -i curl -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -d "%s" --ssl-no-revoke "%s"', new_body, url);
    ...
    json_command = sprintf('env -i curl -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -d '' %s '' --ssl-no-revoke ''%s''', body, url);
